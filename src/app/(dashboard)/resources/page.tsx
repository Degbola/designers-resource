      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            ...
            className={`p-2 rounded-lg border transition-colors cursor-pointer ${showFavoritesOnly ? 'bg-yellow-500/20 border-yellow-500/50 text-yellow-400' : 'bg-dark-700 border-dark-600 text-dark-400 hover:text-white'}`}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((resource) => {
            ...
            return (
              <Card key={resource.id} className="hover:border-dark-500 transition-colors flex flex-col">
                ...
                {resource.tags && (
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {resource.tags.split(',').map((tag, i) => (
                      <span key={i} className="text-xs bg-dark-600 text-dark-200 px-2 py-0.5 rounded">{tag.trim()}</span>
                    ))}
                  </div>
                )}
                <div className="mt-auto pt-3 border-t border-dark-600 flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(resource)} className="p-1.5 rounded-lg hover:bg-dark-700 text-dark-400 hover:text-white transition-colors cursor-pointer"><Edit2 size={14} /></button>
                    <button onClick={() => handleDelete(resource.id)} className="p-1.5 rounded-lg hover:bg-dark-700 text-dark-400 hover:text-red-400 transition-colors cursor-pointer"><Trash2 size={14} /></button>
                  </div>
                </div>
              </Card>
            )
