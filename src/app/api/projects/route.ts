...
export async function POST(req: NextRequest) {
  ...
  const result = await db.prepare(
    `INSERT INTO projects (client_id, name, description, status, priority, start_date, due_date, budget, progress, drive_folder_url)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
...
export async function PUT(req: NextRequest) {
  ...
  await db.prepare(
    ...
    WHERE id=?`
  ).bind(body.client_id, body.name, body.description, body.status, body.priority, body.start_date, body.due_date, body.budget, body.progress, body.drive_folder_url || '', body.id).run()
