          ...
          return (
            <div key={col.id}>
              <div className={`flex items-center gap-2 mb-3 pb-2 border-b-2 ${col.color}`}>
                ...
                <span className="text-xs text-dark-400 bg-dark-700 px-2 py-0.5 rounded-full">{colProjects.length}</span>
              </div>
              <div className="space-y-3">
                {colProjects.map((project) => (
                  <Card key={project.id} className="!p-4 hover:border-dark-500 transition-colors">
                    <div className="flex items-start justify-between mb-2">
                      <Link href={`/projects/${project.id}`} className="font-medium text-white text-sm hover:text-accent transition-colors">{project.name}</Link>
                      <div className="flex gap-1">
                        <button onClick={() => openEdit(project)} className="p-1 rounded hover:bg-dark-600 text-dark-400 hover:text-white cursor-pointer"><Edit2 size={12} /></button>
                        <button onClick={() => handleDelete(project.id)} className="p-1 rounded hover:bg-dark-600 text-dark-400 hover:text-red-400 cursor-pointer"><Trash2 size={12} /></button>
                      </div>
                    </div>
                      ...
                      <div className="flex gap-1 mt-3">
                        {statusColumns.filter((s) => s.id !== project.status).map((s) => (
                          <button key={s.id} onClick={() => updateStatus(project, s.id)}
                            className="text-[10px] px-2 py-1 rounded bg-dark-600 text-dark-300 hover:bg-dark-500 hover:text-white transition-colors cursor-pointer">
                            {s.label}
                          </button>
                        ))}
                      </div>
